	...
	for (int i = 0; i < H; i++)//경계 검출
	{
		for (int j = 0; j < W; j++)
		{
			if (Bin[i * W + j] == 0) //전경화소라면?
			{
				...
				{
					Out[i * W + j] = 255; //경계는 255(흰) 0(검정)으로 출력
				}
...
void DrawRectOutline(BYTE* Img, int W, int H )
{
...
void DrawCrossLine(BYTE* Img, int W, int H)
{
	int sumx = 0;
	int sumy = 0;
	int cnt = 0;
...
int main()
{
	...
	DrawRectOutline(Output, W, H);
	DrawCrossLine(Output, W, H);
